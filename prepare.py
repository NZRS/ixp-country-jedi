...
def get_asns(cand_list):
    ...
    if len(nums) <= len(cand_list) / 4:
      # print "num / cand_list %s/%s : %s" % ( len(nums) , len(cand_list),
      # nums )
      return False
    # sanity-check, to see if a column is not just something with low numbers.
    # woodynet/AS42 is always there ;)
    if max(nums) < 42:
        return []
    return nums
...
def do_probe_selection(probes, conf, basedata):
    ...
    for prb_id in probes:
        ...
        if 'tags' in prb_info and\
                'system-auto-geoip-country' in prb_info['tags']:
        ...
        if 'location-constraint' in basedata and not loc_close_enough:
        ...
        if asn is None:
            print >>sys.stderr, "no ASN for probe?!, what's up RIPE Atlas " \
                                "backend?. %s" % prb_info
            continue
        if asn not in probes_per_asn:
            probes_per_asn[asn] = []
    ...
    for asn in probes_per_asn:
        ...
        # In principle this selects the closest and furthest probe
        # for each of the list of locations
        if len(probes_per_asn[asn]) <= 2*len(basedata['locations']):
            ...
            for loc in basedata['locations']:
                loc_sorted = sorted(probes_per_asn[asn],
                                    key=lambda k: k['dists'][loc])
        ...
        selected_probes = selected_probes + [p for p in selected_asn_probes[asn]]
...
def alexa_country_top25(countries):
    ...
    WARNING: please consider looking at the 'targets' list in basedata.json and see if any of the sites"
    WARNING: that are going to be measured to might be offensive. Please be considerate to our RIPE Atlas Probe hosts"
    Targets found: %s\n""" % '\n'.join(targets)
    time.sleep(5)
    return targets
...
def hitlist_from_websites(urls):
    ...
    for url in urls:
        ...
        for link in soup.findAll('a', href=True):
            ...
            # remove all but full-qualified urls (get rid of website
            # internal stuff typically)
            if ex_urlp.scheme not in ['http','https']:
                continue
            if ex_urlp.hostname == req_urlp.hostname:
                continue
            # remove everything that has url-path (typically website
            # internal stuff)
...
if __name__ == '__main__':
   ...
   ####
    ...
    # TODO: better syntax checking etc. here
    else:
    ...
    if 'locations' not in conf or len(conf['locations']) == 0:
